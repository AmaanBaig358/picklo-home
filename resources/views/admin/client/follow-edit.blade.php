...
=======
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('admin.client.followUp') }}">Client Follow-Ups</a></li>
>>>>>>> 721f0e5 (First commit)
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{ route('admin.update.followup', $followUp->faker_id) }}" method="POST" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="user_id" class="form-label">Client Name</label>
                                        <input type="text" name="client_name" class="form-control" value="{{ $client->client_name }}" disabled>
                                    </div>
                                    <div class="mb-3">
                                        <label for="project-title" class="form-label">Follow Title</label>
                                        <input type="text" name="title" class="form-control" value="{{ $followUp->title }}" placeholder="Enter Follow Title" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="project-title" class="form-label">Description</label>
                                        <textarea name="description" class="form-control" placeholder="Enter Project Description" rows="4" required>{{ $followUp->description }}</textarea>
                                    </div>
                                    <div class="row mt-3">
                                        <div class="col-lg-12">
                                            ...
                                            <div class="upload-container" onclick="document.getElementById('fileInput1').click()">
                                                ...
                                                <div class="preview-container" id="previewContainer1">
                                                    @php
                                                        $uploadedDocuments = json_decode($followUp->upload_files, true) ?? []; // Decode JSON safely
                                                    @endphp
                                                    @foreach($uploadedDocuments as $document)
                                                        @if(preg_match('/\.(jpg|jpeg|png|gif)$/i', $document))
                                                            <img src="{{ asset($document) }}" alt="Uploaded Image"
                                                                 width="100">
                                                        @else
                                                            <a href="{{ asset($document) }}" target="_blank">View
                                                                Document</a>
                                                        @endif
                                                    @endforeach
                                                </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-lg-6">
                                            ...
                                            <input type="datetime-local" id="reminder_date" name="reminder_date" class="form-control" value="{{ $followUp->reminder_date }}" required>
                                        </div>
                                        <div class="col-lg-6">
